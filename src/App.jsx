...
const C = {
  ...
  red: "#c91a1a", redB: "#e53030", redDim: "#c91a1a15", gold: "#c8920e", green: "#22c55e",
  text: "#eceef8", muted: "#5a5272", mutedL: "#8a82a0",
};
...
function ProductImage({ p }) {
  ...
  if (!candidates.length || idx >= candidates.length) return <div style={{ fontSize: 42, opacity: 0.22 }}>🛠️</div>;
  return <img src={candidates[idx]} alt={p.name} onError={() => setIdx((i) => i + 1)} style={{ width: "100%", height: "100%", objectFit: "cover" }} />;
}

function Header({ search, setSearch }) {
  return <header style={{ background: `${C.surface}ee`, borderBottom: `1px solid ${C.borderRed}`, position: "sticky", top: 0, zIndex: 10 }}>
    <div style={{ maxWidth: 1400, margin: "0 auto", padding: "0 20px", minHeight: 64, display: "flex", alignItems: "center", gap: 14 }}>
      ...
      <div><div style={{ fontSize: 15, fontWeight: 900 }}>Thiên Minh Group</div><div style={{ fontSize: 10, color: C.redB, fontWeight: 800, letterSpacing: 1.3 }}>DEKTON® OFFICIAL</div></div>
      <div style={{ flex: 1 }} />
      <div style={{ width: "min(420px,48vw)", position: "relative" }}>
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Tìm sản phẩm, SKU..." style={{ width: "100%", background: C.card, border: `1px solid ${C.borderRed}`, borderRadius: 10, padding: "10px 12px 10px 38px", color: C.text, outline: "none" }} />
      </div>
    </div>
  </header>;
}
...
function BannerGrid({ banners }) {
  ...
  return <div style={{ display: "flex", flexDirection: "column", gap: 12, marginBottom: 18 }}>
    {rows.map((row) => <div key={row} style={{ display: "flex", gap: 12 }}>
      {visible.filter((b) => (b.row || 1) === row).map((b) => <div key={b.id} style={{ flex: b.flex || 1, minHeight: b.height || 140, borderRadius: 16, overflow: "hidden", position: "relative", border: `1px solid ${C.borderRed}`, background: b.bg || "linear-gradient(135deg,#1a0404,#0e0816)" }}>
...
function navBtn(active) {
  return { width: "100%", display: "block", textAlign: "left", marginBottom: 7, padding: "10px 12px", borderRadius: 10, border: `1px solid ${active ? C.borderRed : "transparent"}`, background: active ? C.redDim : "transparent", color: active ? C.redB : C.mutedL, fontWeight: active ? 800 : 500, cursor: "pointer" };
}

function Sidebar({ cats, selected, setSelected }) {
  return <aside style={{ width: 220, flexShrink: 0, paddingRight: 16 }}>
    ...
    {cats.map((cat) => <button key={cat} onClick={() => setSelected(cat)} style={navBtn(selected === cat)}>📦 {cat}</button>)}
  </aside>;
}

function ProductCard({ p, onOrder }) {
  return <div style={{ background: C.card, border: `1px solid ${C.borderRed}`, borderRadius: 16, overflow: "hidden", display: "flex", flexDirection: "column", minHeight: 360 }}>
...
function inputStyle() { return { width: "100%", background: C.surface, border: `1px solid ${C.borderRed}`, borderRadius: 10, padding: "10px 12px", color: C.text, outline: "none" }; }
...
function OrderModal({ product, onClose }) {
  ...
  const submit = async () => {
    setStatus("sending");
    try {
      const res = await apiPost("/api/public/orders", { ...form, qty: Number(form.qty) || 1, product_name: product.name, product_sku: product.sku, total });
      setStatus(`Đã gửi đơn ${res.order_id || ""}. Shop sẽ liên hệ xác nhận.`);
    } catch (e) { setStatus(`Lỗi gửi đơn: ${e.message}`); }
  };
  if (!product) return null;
  return <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,.82)", display: "flex", alignItems: "center", justifyContent: "center", padding: 18 }} onClick={onClose}>
    <div style={{ width: 440, background: C.card, border: `1px solid ${C.borderRed}`, borderRadius: 18, padding: 20 }} onClick={(e) => e.stopPropagation()}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, marginBottom: 14 }}><div><div style={{ fontSize: 16, fontWeight: 900 }}>Đặt hàng / Tư vấn</div><div style={{ fontSize: 12, color: C.mutedL, marginTop: 4 }}>{product.name}</div></div><button onClick={onClose} style={{ background: "transparent", border: "none", color: C.muted, fontSize: 24, cursor: "pointer" }}>×</button></div>
      {[["name", "Họ tên"], ["phone", "Số điện thoại"], ["address", "Địa chỉ giao hàng"]].map(([k, label]) => <div key={k} style={{ marginBottom: 10 }}><div style={{ fontSize: 10, color: C.muted, fontWeight: 800, marginBottom: 4 }}>{label}</div><input value={form[k]} onChange={(e) => upd(k, e.target.value)} style={inputStyle()} /></div>)}
      <div style={{ marginBottom: 12 }}><div style={{ fontSize: 10, color: C.muted, fontWeight: 800, marginBottom: 4 }}>Số lượng</div><input type="number" min="1" value={form.qty} onChange={(e) => upd("qty", e.target.value)} style={inputStyle()} /></div>
...
function Footer({ settings }) {
  ...
  return <footer style={{ marginTop: 44, background: C.surface, borderTop: `1px solid ${C.borderRed}` }}><div style={{ maxWidth: 1400, margin: "0 auto", padding: "30px 20px", display: "grid", gridTemplateColumns: "2fr 1fr 1.4fr", gap: 24 }}>
...
export default function App() {
  ...
  const [loading, setLoading] = useState(true), [err, setErr] = useState(""), [search, setSearch] = useState(""), [cat, setCat] = useState("all"), [orderProduct, setOrderProduct] = useState(null);
  useEffect(() => { let alive = true; (async () => { setLoading(true); setErr(""); try { const [p, b, s] = await Promise.all([apiGet("/api/public/products"), apiGet("/api/public/banners"), apiGet("/api/public/settings")]); if (!alive) return; setProducts(Array.isArray(p.products) ? p.products : []); setBanners(Array.isArray(b.banners) ? b.banners : []); setSettings(s || null); } catch (e) { if (alive) setErr(e.message || "Không tải được dữ liệu"); } finally { if (alive) setLoading(false); } })(); return () => { alive = false; }; }, []);
  const cats = useMemo(() => [...new Set(products.map((p) => p.cat || "Khác"))].sort(), [products]);
  const shown = useMemo(() => { const q = search.trim().toLowerCase(); return products.filter((p) => (cat === "all" || (p.cat || "Khác") === cat) && (!q || `${p.name} ${p.sku} ${p.desc}`.toLowerCase().includes(q))); }, [products, cat, search]);
  return <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "system-ui,sans-serif" }}><style>{`*{box-sizing:border-box}body{margin:0;background:${C.bg}}button{transition:.15s}button:hover{filter:brightness(1.08)}::-webkit-scrollbar{width:6px}::-webkit-scrollbar-thumb{background:#2c1010;border-radius:99px}`}</style><Header search={search} setSearch={setSearch} /><main style={{ maxWidth: 1400, margin: "0 auto", padding: 20 }}><BannerGrid banners={banners} />{err && <div style={{ background: "#3a1111", border: `1px solid ${C.redB}`, color: C.redB, borderRadius: 12, padding: 12, marginBottom: 16 }}>⚠️ {err}</div>}{loading ? <div style={{ padding: 50, textAlign: "center", color: C.mutedL, background: C.card, borderRadius: 16, border: `1px solid ${C.border}` }}>Đang tải dữ liệu từ hệ thống...</div> : <div style={{ display: "flex", alignItems: "flex-start" }}><Sidebar cats={cats} selected={cat} setSelected={setCat} /><section style={{ flex: 1 }}><div style={{ display: "flex", justifyContent: "flex-start", alignItems: "center", marginBottom: 14 }}><div style={{ color: C.mutedL, fontSize: 13 }}>{shown.length} sản phẩm {cat !== "all" ? `trong ${cat}` : ""}</div></div>{!shown.length ? <div style={{ padding: 50, textAlign: "center", color: C.muted, background: C.card, borderRadius: 16 }}>Không tìm thấy sản phẩm</div> : <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(230px,1fr))", gap: 14 }}>{shown.map((p) => <ProductCard key={`${p.id}-${p.sku}`} p={p} onOrder={setOrderProduct} />)}</div>}</section></div>}</main><Footer settings={settings} /><OrderModal product={orderProduct} onClose={() => setOrderProduct(null)} /></div>;
}
